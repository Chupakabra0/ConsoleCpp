...
static bool bisearch(char32_t ucs, const struct interval *table, int max) {
	int min = 0;
	int mid;
 ...
 * in ISO 10646.
 */
static bool mk_is_wide_char(char32_t ucs) {
  ...
  return bisearch(ucs, wide, sizeof(wide) / sizeof(struct interval) - 1);
}
